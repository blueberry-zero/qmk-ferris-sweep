...
### Flashing Wave-1 Keyboard
  1. qmk flash -kb ferris/sweep -km <NAME_UR_KEYBOARD> -e CONVERT_TO=promicro_rp2040
  2. When the prompt says waiting for device, short RST and GRND for >1s with a paper clip
  3. Open the directory in thunar (can't find it in /media/skhan folder in cli?)

### Defining Layers for my wiring

- Each layer is defined as an array within the "layers" field
- The individual "layer" array is a single dimensional array that is interpreted with the following rules
  - There are 4 rows
  - Row 1 has 10 keys and starts at layer[0]
  - Row 2 has 10 keys and starts at layer[10]
  - Row 3 has 10 keys and starts at layer[20]
  - Row 4 has 4 keys and starts at layer [3-]
  - Key presses are defined by character strings representing KeyCodes, see full list on [QMK](https://github.com/qmk/qmk_firmware/blob/master/docs/keycodes.md)
  - See the folloing notation for ROW_COL with 0,0 being the TOP RIGHT of the keyboard
...
### Default Documentation for mini-ryoku
